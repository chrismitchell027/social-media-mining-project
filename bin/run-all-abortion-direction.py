...
def find_abortion_csvs(input_dir):
    """
    Find all 'Abortion' CSV files (Topic_State.csv) in the directory,
    ...
    for fname in os.listdir(input_dir):
        ...
        # Expect format 'Abortion_STATE'
        if not root.startswith('Abortion_'):
            continue
...
def main():
    ...
    try:
        ...
        csv_files = find_abortion_csvs(input_dir)
        if not csv_files:
            print("No abortion CSV files found to process.")
            sys.exit(0)
